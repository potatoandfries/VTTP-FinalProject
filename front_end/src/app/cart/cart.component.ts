...
export class CartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.cartItemCountSubscription = this.cartState.subscribe(data => {
      if (data && data.cart && data.cart.cartItems && data.cart.cartItems.length) {
  ...
  applyCode(discountCodeField: string) {
  ...
  activatePurchase() {
    this.store.select('auth')
      ...
      .subscribe(data => {
        // if (data.isActive) {
          this.store.dispatch(OrderActions.isCheckoutActive({ isActive: true }));
