...
import { ProductDetail, Product, ProductVariantResponse, Category, Colors } from '../configurations/model';
import { config } from '../configurations/local';

@Injectable()
export class ProductService {

  private readonly publicUrl = `${config.apiUrl}/api/public/product`;
  private readonly categoryUrl = `${config.apiUrl}/api/public/category`;
  private readonly colorUrl = `${config.apiUrl}/api/public/colors`;

  private readonly browsePageSize = 20;
  ...
  getRelatedProducts(productUrl: string) {
    return this.httpClient.get<Product[]>(`${this.publicUrl}/related/${productUrl}`);
  }

  getNewlyAdded() {
    return this.httpClient.get<Product[]>(`${this.publicUrl}/recent`);
  }

  getMostSelling() {
    return this.httpClient.get<ProductVariantResponse[]>(`${this.publicUrl}/mostselling`);
  }

  getInterested() {
    return this.httpClient.get<Product[]>(`${this.publicUrl}/interested`);
  }
  ...
  getColors() {
    return this.httpClient.get<Colors[]>(this.colorUrl);
  }
}
