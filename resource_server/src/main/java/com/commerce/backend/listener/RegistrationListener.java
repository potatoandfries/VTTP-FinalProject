...
public class RegistrationListener implements ApplicationListener<OnRegistrationCompleteEvent> {
    ...
    @Override
    public void onApplicationEvent(@SuppressWarnings("null") OnRegistrationCompleteEvent event) {
        this.confirmRegistration(event);
    }

    private void confirmRegistration(OnRegistrationCompleteEvent event) {
        String recipientAddress = event.getUser().getEmail();
        String subject = "\uD83D\uDD11 shoely Registration Confirmation";
        String confirmationUrl = mailConstants.getHostAddress() + "/validate-email?token=" + event.getToken();
